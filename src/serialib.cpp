...
char serialib::openDevice(const char *Device, const unsigned int Bauds,
                          SerialDataBits Databits, SerialParity Parity,
  ...
  options.c_cflag |=
      (CLOCAL | CREAD | databits_flag | parity_flag | stopbits_flag);
...
/*!
     ...
     \param timeOut_ms : delay of timeout before giving up the reading
   (optional)
     \return  >0 success, return the number of bytes read (including the null
   character)
  ...
  while (nbBytes < maxNbBytes) {
    ...
    if (timeOutParam > 0) {
      ...
      if (charRead == 1) {
        ...
        // This is not the final character, just increase the number of bytes
        // read
        nbBytes++;
      }
...
/*!
    ...
    Note that when using serial over USB on Unix systems, a delay of 20ms may be
   necessary before calling the flushReceiver function
